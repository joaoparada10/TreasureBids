@php
    use Carbon\Carbon;
@endphp
<div class="profile-auctions followed-auctions {{ request('followedAuctionsPage') ? 'selected' : '' }}">
    <h2 class="text-secondary">Followed Auctions</h2>
    @if(isset($followedAuctions) && $followedAuctions->isEmpty())
    <p class="text-muted">No live auctions you're following.</p>
    @else
    <ul class="list-group">
    ...
    <div class="mt-3">
          {{ $followedAuctions->appends(['section' => 'auctions'])->links() }}
      </div>
    @endif
</div>